...
mkdir -p q1-evidence
{
  printf 'git_head='
  git rev-parse HEAD
  printf 'app_apk_sha256='
  sha256sum "$APK"
  printf 'public_package=%s\n' "$PACKAGE"
  printf 'launchable_activity=%s\n' "$YAW_ACTIVITY_CLASS"
  printf 'public_app_label=%s\n' "$APP_LABEL"
} > q1-evidence/exact-candidate-identity.txt
...
wait_for_android_ready() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    # API 36 Google APIs images may ship NexusLauncher instead of AOSP Launcher3.
    # Package-manager readiness is independent of which HOME implementation ships.
    local framework
    framework="$(adb shell pm path android 2>/dev/null | tr -d '\r' || true)"
    if [ "$boot" = "1" ] && grep -q '^package:' <<<"$framework"; then
      echo "Android boot/package services ready (framework package resolved)."
      adb shell pm list packages 2>/dev/null | grep -Ei 'launcher|android
    sleep 3
  done
  echo "Android did not reach boot/package-service readiness: boot=$boot framework=$framework" >&2
  adb shell pm list packages > q1-evidence/packages-not-ready.txt 2>&1 || true
  adb shell cmd package resolve-activity --brief -a android.intent.action.MAIN \
    -c android.intent.category.HOME > q1-evidence/home-resolution-not-ready.txt 2>&1 || true
...
echo "Q1 API36 build/install/visible-shell/real launcher widget-placement smoke PASS."
 \
        > q1-evidence/package-home-candidates.txt || true
      return 0
    fi
    sleep 3
  done
